...
from tensorflow.keras.layers import Activation, Add, Conv2D, Input, Lambda, concatenate
from tensorflow.keras.models import Model

from src_super_resolution.imagemodel import ImageModel
...
def get_network(weights):
    if weights in WEIGHTS_URLS.keys():
        ...
        raise ValueError(
            "Available RRDN network weights: {}".format(list(WEIGHTS_URLS.keys()))
        )
    c_dim = 3
    kernel_size = 3
