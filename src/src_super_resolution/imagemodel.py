...
from src_super_resolution.image_processing import (
    process_array,
    process_output,
...
class ImageModel:
    ...
    def predict(
        self, input_image_array, by_patch_of_size=None, batch_size=10, padding_size=2
    ):
        """
        ...
        if by_patch_of_size:
            ...
            scaled_image_shape = tuple(
                np.multiply(input_image_array.shape[0:2], scale)
            ) + (3,)
