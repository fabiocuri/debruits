...
_SUBHEADS = {"Definition", "How it works", "Example", "Further reading"}
...
def _article(text: str, rtl: bool = False) -> Markup:
    ...
    def flush() -> None:
        if para:
            # The very first paragraph (before any sub-head) is the concept title.
            cls = ' class="concept"' if not blocks else ""
            blocks.append(f"<p{cls}>" + " ".join(para) + "</p>")
            para.clear()
...
def index(request: Request):
    ...
    return templates.TemplateResponse(
        "index.html",
        {
            ...
            "agenda_configured": bool(config.CALENDAR_ICS_URL or config.TEAMS_ICS_URL),
