...
TASKS = [
    {
        ...
        "max_tokens": 1000,
        "prompt": (
            "I'm a software / AI engineer building broad technical fundamentals. Teach "
            "me ONE self-contained technical concept — for example VPN, MCP (Model "
            "Context Protocol), HTTP/HTTPS, DNS, TLS, OAuth, containers, load "
            "balancing, message queues, caching, webhooks, or similar. Rotate widely "
            "and pick a different one each time; do NOT pick GPU / CUDA / NVIDIA or "
            "hardware topics. Be genuinely technical but clear. Output plain text with "
            "the concept name ALONE on the first line, then exactly these sections, "
            "each header on its own line:\n"
            "Definition\nHow it works\nExample\nFurther reading\n"
            "Under 'Definition': one or two precise sentences. Under 'How it works': a "
            "short technical explanation (mention protocols, ports, layers, handshakes, "
            "etc. where relevant). Under 'Example': one concrete real-world example. "
            "Under 'Further reading': a single real https:// link to documentation or a "
            "reputable article. Begin with the concept-name line and do not restate or "
